...
import { sleep } from '../models/types'

function parseGtx(data: unknown, fallbackSource: string) {
  const d = data as [[string][][], unknown, string]
  return { translated: (d?.[0] ?? []).map((part) => part[0]).join(''), source: d?.[2] ?? fallbackSource }
}
...
async function translateChromeEx(q: string, source: string, target: string) {
  const url = 'https://clients5.google.com/translate_a/t' + `?client=dict-chrome-ex&sl=${encodeURIComponent(source)}&tl=${encodeURIComponent(target)}&q=${encodeURIComponent(q)}`
  try {
    const res = await fetch(url, { headers: { 'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64)' } })
    if (!res.ok) return null
    const data = (await res.json()) as [string, string][]
    return { translated: data.map((part) => part[0]).join(''), source: data[0]?.[1] ?? source }
  } catch { return null }
}
